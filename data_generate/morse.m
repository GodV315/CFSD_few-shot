...
function [Dit] = Dit()
    ...
    global Fc_global;
    global jitter_global;
    global drifting_global;   
    t_dit = 0: 1/Fs_global: time_dit_global*unifrnd(0.7,1.5,1,1)-1/Fs_global;  % unifrnd函数加时长偏差
    if jitter_global == 0    
        Dit = exp(j*2*pi*(Fc_global).*t_dit);
    else
        Dit = exp(j*2*pi*(Fc_global-randi(jitter_global)*t_dit).*t_dit);
    end
end
...
function [Dah] = Dah()
    ...
    if jitter_global == 0   
        Dah = exp(j*2*pi*(Fc_global).*t_dah);
    else
        Dah = exp(j*2*pi*(Fc_global-randi(jitter_global)*t_dah).*t_dah);
    end
end 